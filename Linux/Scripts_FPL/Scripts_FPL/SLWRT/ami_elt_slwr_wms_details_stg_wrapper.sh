##############################################################################################
##############################################################################################
##################      SCRIPT_NAME   : ami_elt_slwr_wms_details_stg_wrapper.sh	 	 #########
...
Email()
{
	...
	cat ${LogFile} | mailx -s "AMI_DIM_ELT_PROCESS(`hostname`:${MyName}:${RunTs}) | slwr_wms_details_stg | ${MyStatus} | ${MyBatches}" ${EmailList}
}
...
StgTblName1="slwr_wms_details_stg"
...
export RunDate=`date +%Y%m%d`
	...
	sh /home/ami0app/ami_scripts/bin/stage_load_process.sh ${StgTblName1} ${ETLBatchID}>>${LogFile} 2>&1
	RC1=`echo $?`
	LogMe "Data Load to stage table ended with return codes : [$RC1]"

	LogMe "#########################################################################################################################"


if [ ${RC1} -ne 0 ];then
LogMe "Stage load failed for the table : ami_stage.${StgTblName1}."
	Email "Stage Failed" "${ETLBatchID}"
LogMe "#########################################################################################################################"
exit 1
else
	LogMe "stage table loaded successfully."
